...
function addFeedItem({feedID, link}) {
  ...
  Api.Feeds.saveItem({feedID, item}).catch((err) => {
...
function createFeedsMenu() {
  ...
  .then((feeds) => {
    feeds.forEach((feed) => {
      const item = {title: 'add to ' + feed.title, contexts:LINK_CONTEXT, id: 'feeds.' + feed.id};
...
function createSigninMenu() {
  chrome.contextMenus.create({title: 'Sign in to add feeds', contexts:LINK_CONTEXT, id:'signin'});
}
